...
export class SalesforceESGApi extends RequestLogger {
    ...
    while (true) {
      try {
        ...
        if (response.status() !== this.expectedStatus) {
          ...
          console.log(
            color.error(`Unexpected response code: ${response.status()}, retrying...`)
          )
          continue
        }
        ...
        if (resp.totalSize === totalSize) {
          ...
          console.log(
            color.warning(
              `Total records found (${resp.totalSize}) does not match expected (${totalSize}), retrying...`
            )
          )
          continue
        }
  ...
  async getSignUpTransactionId(signUpId: string, totalSize: number = 1): Promise<string> {
    ...
    while (true) {
      try {
        console.log(
          color.info(`Searching for Transactions related to SignUp ID: ${signUpId}...`)
        )
        ...
        if (!transactionRecords || transactionRecords.length === 0) {
          console.log(
            color.warning(`No Transactions found for SignUp ID ${signUpId}, retrying...`)
          )
          continue
        }
      ...
      try {
        ...
        if (response.status() === this.expectedStatus) {
          ...
          if (!resp.hasOwnProperty('attributes')) {
            console.log(
              color.error(`Response does not have "attributes" property, retrying...`)
            )
            continue
          }
          break
          ...
          console.log(
            color.error(
              `Response code is not ${this.expectedStatus}. Got ${response.status()}, retrying...`
            )
          )
          continue
        }
    ...
    while (true) {
      try {
        ...
        if (response.status() === 204) {
          console.log(
            color.success(
              `Object ${objectType} with ID ${objectId} updated successfully.`
            )
          )
          break
        } else {
          ...
          await this.logResponse(response.status(), await response.text())
          console.log(
            color.error(`Response code is not 204. Got ${response.status()}, retrying...`)
          )
          continue
        }
      } catch (err) {
        ...
        try {
          if (foundElement !== undefined) {
            ...
            if (typeof expectedValue === 'boolean' && foundElement === expectedValue) {
              ...
              !isRegexString(expectedValue) &&
              foundElement.includes(expectedValue)
            ) {
              isMatch = true
            } else if (
              typeof expectedValue === 'number' &&
              foundElement === expectedValue
            ) {
              isMatch = true
            } else if (
              typeof expectedValue === 'string' &&
            ...
            if (isMatch) {
              console.log(
                `Found Salesforce property '${path}' with value of ${foundElement}`
              )
              ...
              throw new Error(
                `Found Salesforce property '${path}' but value '${foundElement}' does not match the expected value '${expectedValue}', retrying...`
              )
            }
          } else {
  ...
  async publishToInboundEsgUpdates(payload: any): Promise<string> {
    ...
    try {
      ...
      if (!Type__c || !SubType__c || !Payload__c) {
        throw new Error(
          '❌ Missing required fields: Type__c, SubType__c, or Payload__c in payload'
        )
      }
      ...
      if (status === 201 && responseData.success === true) {
        console.log(
          color.success(`✅ Successfully published! 🎉 Salesforce ID: ${responseData.id}`)
        )
        ...
        console.log(
          color.error(`🚨 Response Details: ${JSON.stringify(responseData, null, 2)}`)
        )
    ...
    for (const id of signupIds) {
      ...
      const missingFieldsArray =
        typeof missingFields === 'string'
          ? missingFields.split(',').map(field => field.trim())
          : missingFields
      ...
      expectedMissingFields.forEach(expectedField => {
        if (!missingFieldsArray.includes(expectedField)) {
          ...
          throw new Error(
            `Expected missing field "${expectedField}" for signup ID "${id}", but it was not found in the response`
          )
        }
      })
  ...
  async waitForStrictLogAssertions(
    ...
    while (Object.keys(remainingAssertions).length > 0) {
      ...
      for (const [jsonPath, expectedValue] of Object.entries(remainingAssertions)) {
        ...
        console.log(
          `🔎 Asserting: ${jsonPath} | Expected: ${expectedValue} | Actual: ${actualValue}`
        )
      ...
      if (Object.keys(remainingAssertions).length > 0) {
        console.log(
          `⏳ Remaining assertions: ${Object.keys(remainingAssertions).length}. Retrying in 5 seconds...`
        )
        await this.delay(5000)
      }
    }
  ...
  async waitForStrictLogRegexAssertions(
    ...
    while (Object.keys(remainingAssertions).length > 0) {
      ...
      for (const [jsonPath, expectedRegex] of Object.entries(remainingAssertions)) {
        ...
        console.log(
          `🔎 Asserting: ${jsonPath} | Expected Regex: ${expectedRegex} | Actual: ${actualValue}`
        )
  ...
  async waitForNonEmptyLogAssertions(
    ...
    while (remainingAssertions.size > 0) {
      ...
      for (const jsonPath of remainingAssertions) {
        ...
        console.log(
          `🔎 Checking: ${jsonPath} | Actual: ${JSON.stringify(actualValue)} | Valid: ${isValid}`
        )
        ...
        if (isValid) {
          ...
          console.log(
            `❌ Assertion failed for ${jsonPath}. Value is empty, null, or undefined. Retrying...`
          )
        }
      }
      ...
      if (remainingAssertions.size > 0) {
        console.log(
          `⏳ Remaining assertions: ${remainingAssertions.size}. Retrying in 5 seconds...`
        )
        await this.delay(5000)
      }
    }
  ...
  async waitForNullValueAssertions(
    ...
    while (remainingAssertions.size > 0) {
      ...
      for (const jsonPath of remainingAssertions) {
        ...
        console.log(
          `🔎 Checking: ${jsonPath} | Actual: ${JSON.stringify(actualValue)} | Is Null: ${isNull}`
        )
        ...
        if (isNull) {
          ...
          console.log(
            `❌ Assertion failed for ${jsonPath}. Value is not NULL. Retrying...`
          )
        }
      }
